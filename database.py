...
def init_db():
    ...
    if c.fetchone()["n"] == 0:
        # image_url left empty on purpose: the web app shows a nice gradient+emoji
        # placeholder until the admin uploads a real cover via /admin -> "Yangi o'yin qo'shish",
        # or you replace these rows directly in the games table later.
        default_games = [
            ("PUBG Mobile", "", "rounded"),
            ("Free Fire", "", "rounded"),
            ("Mobile Legends", "", "rounded"),
            ("Honor of Kings", "", "rounded"),
            ("Standoff 2", "", "rounded"),
        ]
